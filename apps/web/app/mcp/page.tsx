import Link from "next/link";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "MCP - v1.run",
  description: "Connect v1.run to your AI agent via Model Context Protocol",
};

export default function MCPPage() {
  return (
    <main className="min-h-screen bg-black text-white font-mono">
      {/* Content */}
      <div className="container-page py-12">
        <h1 className="text-2xl font-bold mb-4">Model Context Protocol</h1>
        <p className="text-[#888] mb-8">
          Give your AI agent fast, accurate npm data — latest versions, security vulnerabilities, and package health.
        </p>

        {/* Why */}
        <section className="mb-12">
          <h2 className="text-xs uppercase tracking-widest text-[#666] mb-4">Why</h2>
          <div className="space-y-3 text-sm text-[#888]">
            <p><span className="text-white">Fast</span> — Sub-100ms responses from our indexed database</p>
            <p><span className="text-white">Accurate</span> — Real-time version data, no hallucinated packages</p>
            <p><span className="text-white">Secure</span> — Vulnerability data from OSV for every package</p>
            <p><span className="text-white">Current</span> — Synced with npm registry, always up to date</p>
          </div>
        </section>

        {/* Installation */}
        <section className="mb-12">
          <h2 className="text-xs uppercase tracking-widest text-[#666] mb-4">Configuration</h2>
          
          {/* Cursor deeplink */}
          <div className="mb-6">
            <Link
              href="https://cursor.com/link/prompt?text=Add%20v1.run%20MCP%20server%20to%20my%20Cursor%20configuration.%20Add%20this%20to%20.cursor%2Fmcp.json%3A%0A%0A%7B%0A%20%20%22mcpServers%22%3A%20%7B%0A%20%20%20%20%22v1%22%3A%20%7B%0A%20%20%20%20%20%20%22url%22%3A%20%22https%3A%2F%2Fapi.v1.run%2Fmcp%22%0A%20%20%20%20%7D%0A%20%20%7D%0A%7D"
              target="_blank"
              className="inline-flex items-center gap-2.5 px-4 py-2.5 bg-black border border-[#333] rounded-lg text-sm font-medium text-white hover:bg-[#111] hover:border-[#444] transition-colors"
            >
              {/* Cursor logo icon */}
              <svg width="18" height="18" viewBox="0 0 130 145" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill="currentColor" d="M 60.66 0.00 L 64.42 0.00 C 82.67 10.62 100.99 21.12 119.25 31.72 C 121.24 33.01 123.54 34.18 124.72 36.34 C 125.34 39.17 125.06 42.10 125.11 44.98 C 125.07 63.63 125.08 82.28 125.11 100.93 C 125.07 102.84 125.14 104.79 124.73 106.68 C 123.53 108.54 121.50 109.62 119.68 110.77 C 104.03 119.72 88.45 128.80 72.85 137.83 C 69.53 139.68 66.36 141.91 62.74 143.17 C 60.51 142.85 58.57 141.57 56.62 140.54 C 40.81 131.22 24.82 122.22 9.00 112.92 C 5.85 111.16 2.79 109.23 0.00 106.93 L 0.00 36.10 C 3.83 32.32 8.81 30.12 13.34 27.33 C 29.10 18.19 44.82 8.98 60.66 0.00 M 5.62 38.04 C 8.28 40.64 11.88 41.83 14.96 43.80 C 30.60 53.06 46.50 61.89 62.05 71.30 C 62.86 75.82 62.50 80.43 62.55 85.00 C 62.57 100.64 62.51 116.29 62.54 131.93 C 62.54 133.69 62.72 135.44 63.01 137.17 C 64.18 135.60 65.29 133.98 66.24 132.27 C 83.07 103.08 99.93 73.92 116.65 44.67 C 117.89 42.62 118.84 40.42 119.57 38.14 C 113.41 37.65 107.23 37.91 101.06 37.87 C 73.71 37.85 46.35 37.85 18.99 37.87 C 14.54 38.02 10.07 37.53 5.62 38.04 Z"/>
              </svg>
              Add to Cursor
            </Link>
          </div>

          <p className="text-sm text-[#888] mb-4">
            Or manually add to <code className="text-[#ccc]">.cursor/mcp.json</code>:
          </p>
          <div className="bg-[#111] border border-[#333] p-4 overflow-x-auto">
            <pre className="text-sm text-[#ccc]">
{`{
  "mcpServers": {
    "v1": {
      "url": "https://api.v1.run/mcp"
    }
  }
}`}
            </pre>
          </div>
        </section>

        {/* Available Tools */}
        <section className="mb-12">
          <h2 className="text-xs uppercase tracking-widest text-[#666] mb-4">Tools</h2>
          <div className="space-y-4">
            <div className="border border-[#333] p-4">
              <h3 className="text-white font-medium mb-2">package_health</h3>
              <p className="text-sm text-[#888]">
                Latest version, vulnerabilities, health score, downloads, TypeScript support, and maintenance status.
              </p>
            </div>
            <div className="border border-[#333] p-4">
              <h3 className="text-white font-medium mb-2">search_packages</h3>
              <p className="text-sm text-[#888]">
                Fast search across 3M+ packages. Filter by downloads, types, and category.
              </p>
            </div>
            <div className="border border-[#333] p-4">
              <h3 className="text-white font-medium mb-2">compare_packages</h3>
              <p className="text-sm text-[#888]">
                Side-by-side comparison of health, security, and popularity.
              </p>
            </div>
          </div>
        </section>

        {/* Links */}
        <section>
          <h2 className="text-xs uppercase tracking-widest text-[#666] mb-4">Links</h2>
          <div className="space-y-2 text-sm">
            <Link
              href="https://api.v1.run/mcp"
              target="_blank"
              className="block text-[#888] hover:text-white transition-colors"
            >
              MCP Endpoint ↗
            </Link>
            <Link
              href="https://modelcontextprotocol.io"
              target="_blank"
              className="block text-[#888] hover:text-white transition-colors"
            >
              MCP Documentation ↗
            </Link>
          </div>
        </section>
      </div>

      {/* Footer */}
      <footer className="border-t border-[#333] mt-12">
        <div className="container-page py-4 text-xs text-[#666]">
          <Link href="/" className="hover:text-white transition-colors">
            v1.run
          </Link>
        </div>
      </footer>
    </main>
  );
}
