...
describe("Compare API", () => {
  test("GET /api/compare/packages?names= compares multiple packages", async () => {
    const res = await request("/api/compare/packages?names=react,vue");
  ...
  test("GET /api/compare/categories returns all categories", async () => {
    const res = await request("/api/compare/categories");
    expect(res.status).toBe(200);
